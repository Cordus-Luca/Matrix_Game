...
void Update() {
  ...
  // UPDATE THE MATR
  for (j = 0; j < snake.len; j++) {
    matr[snake.body[j][0]] = matr[snake.body[j][0]] + (128 >> snake.body[j][1]); // SE SHIFTEAZA COLOANA
  }

  matr[apple.rPos] =  matr[apple.rPos] + (128 >> apple.cPos);
}
